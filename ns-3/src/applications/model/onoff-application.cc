...
void OnOffApplication::SetTracersForTCP() {
	needTracersForTCP = true;
}
  ...
  if (!m_socket)
    {
      ...
      if (needTracersForTCP) {
    	  Ptr<TcpSocketBase> tcpSock = DynamicCast<TcpSocketBase>(m_socket);
    	  tcpSock->TraceConnectWithoutContext("CongestionWindow",
    			  	  	  MakeCallback(&OnOffApplication::CWndTracer, this));
      }
    }
...
void OnOffApplication::CWndTracer(uint32_t oldVal, uint32_t newVal)
{
	cwndChanges.push_back(::std::tuple<::ns3::Time, uint32_t, uint32_t>(Simulator::Now(), oldVal, newVal));
}
