...
public:
  ...
  /** \return Face to which this Li, double snr = 0.0nkService is attached
   */
  const Face*
  getFace() const;
...
public: // lower interface to be invoked by Transport
   ...
   */
  void
  receivePacket(const Block& packet, const EndpointId& endpoint, double snr = 0.0, double rssi = 0.0);
...
protected: // upper interface to be invoked in subclass (receive path termination)
   ...
   */
  void
  receiveInterest(const Interest& interest, const EndpointId& endpoint, double snr = 0.0, double rssi = 0.0);

  /** \brief delivers received Data to forwarding
   */
  void
  receiveData(const Data& data, const EndpointId& endpoint, double snr = 0.0, double rssi = 0.0);

  /** \brief delivers received Nack to forwarding
   */
  void
  receiveNack(const lp::Nack& nack, const EndpointId& endpoint, double snr = 0.0, double rssi = 0.0);
...
private: // lower interface to be overridden in subclass
  virtual void
  doReceivePacket(const Block& packet, const EndpointId& endpoint, double snr = 0.0, double rssi = 0.0) = 0;
...
inline void
LinkService::receivePacket(const Block& packet, const EndpointId& endpoint, double snr, double rssi)
{
  doReceivePacket(packet, endpoint, snr, rssi);
}
