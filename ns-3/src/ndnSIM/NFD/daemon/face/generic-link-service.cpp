...
GenericLinkService::encodeLpFields(const ndn::PacketBase& netPkt, lp::Packet& lpPacket)
{
  ...
  shared_ptr<lp::LltcConsumerIdTag> lltcConsumerIdTag = netPkt.getTag<lp::LltcConsumerIdTag>();
  if (lltcConsumerIdTag != nullptr) {
    lpPacket.add<lp::LltcConsumerIdField>(*lltcConsumerIdTag);
  }
  else {
    lpPacket.add<lp::LltcConsumerIdField>(0);
  }

  shared_ptr<lp::LltcSnrTag> lltcSnrTag = netPkt.getTag<lp::LltcSnrTag>();
  if (lltcSnrTag != nullptr) {
    lpPacket.add<lp::LltcSnrField>(*lltcSnrTag);
  }
  else {
	auto snrTag = make_shared<lp::LltcSnrTag>(std::pair<double, double>({0.0, 0.0}));
    lpPacket.add<lp::LltcSnrField>(*snrTag);
  }
...
void
GenericLinkService::doReceivePacket(const Block& packet, const EndpointId& endpoint, double snr, double rssi)
{
  try {
    ...
    if (isReassembled) {
      this->decodeNetPacket(netPkt, firstPkt, endpoint, snr, rssi);
    }
  }
...
GenericLinkService::decodeNetPacket(const Block& netPkt, const lp::Packet& firstPkt,
                                    const EndpointId& endpointId, double snr, double rssi)
{
  try {
    switch (netPkt.type()) {
      case tlv::Interest:
        if (firstPkt.has<lp::NackField>()) {
          this->decodeNack(netPkt, firstPkt, endpointId, snr, rssi);
        }
        else {
          this->decodeInterest(netPkt, firstPkt, endpointId, snr, rssi);
        }
        break;
      case tlv::Data:
        this->decodeData(netPkt, firstPkt, endpointId, snr, rssi);
        break;
      default:
        ++this->nInNetInvalid;
                                   ...
                                   const EndpointId& endpointId, double snr, double rssi)
{
  ...
  if (firstPkt.has<lp::LltcConsumerIdField>()) {
	  interest->setTag(make_shared<lp::LltcConsumerIdTag>(firstPkt.get<lp::LltcConsumerIdField>()));
  }

  if (firstPkt.has<lp::LltcSnrField>()) {
	  interest->setTag(make_shared<lp::LltcSnrTag>(firstPkt.get<lp::LltcSnrField>()));
  }
  ...
  auto snrTag = make_shared<lp::LltcSnrTag>(std::pair<double, double>({snr, rssi}));
  interest->setTag<lp::LltcSnrTag>(snrTag);

  this->receiveInterest(*interest, endpointId, snr);
}
                               ...
                               const EndpointId& endpointId, double snr, double rssi)
{
  ...
  if (firstPkt.has<lp::LltcConsumerIdField>()) {
	  data->setTag(make_shared<lp::LltcConsumerIdTag>(firstPkt.get<lp::LltcConsumerIdField>()));
  }

  if (firstPkt.has<lp::LltcSnrField>()) {
	  data->setTag(make_shared<lp::LltcSnrTag>(firstPkt.get<lp::LltcSnrField>()));
  }

//  if (firstPkt.has<lp::LltcHopCountField>()) {
//	  data->setTag(make_shared<lp::LltcHopCountTag>(firstPkt.get<lp::LltcHopCountField>()));
//  }
//
//  if (firstPkt.has<lp::LltcRreqIDField>()) {
//	  data->setTag(make_shared<lp::LltcRreqIDTag>(firstPkt.get<lp::LltcRreqIDField>()));
//  }
//
//  if (firstPkt.has<lp::LltcDestNodeIDField>()) {
//	  data->setTag(make_shared<lp::LltcDestNodeIDTag>(firstPkt.get<lp::LltcDestNodeIDField>()));
//  }
//
//  if (firstPkt.has<lp::LltcDestSeqNoField>()) {
//	  data->setTag(make_shared<lp::LltcDestSeqNoTag>(firstPkt.get<lp::LltcDestSeqNoField>()));
//  }
//
//  if (firstPkt.has<lp::LltcOrigNodeIDField>()) {
//	  data->setTag(make_shared<lp::LltcOrigNodeIDTag>(firstPkt.get<lp::LltcOrigNodeIDField>()));
//  }
//
//  if (firstPkt.has<lp::LltcOrigSeqNoField>()) {
//	  data->setTag(make_shared<lp::LltcOrigSeqNoTag>(firstPkt.get<lp::LltcOrigSeqNoField>()));
//  }


  auto snrTag = make_shared<lp::LltcSnrTag>(std::pair<double, double>({snr, rssi}));
  data->setTag<lp::LltcSnrTag>(snrTag);

  this->receiveData(*data, endpointId, snr);
}
                               ...
                               const EndpointId& endpointId, double snr, double rssi)
{
  ...
  auto snrTag = make_shared<lp::LltcSnrTag>(std::pair<double, double>({snr, rssi}));
  nack.setTag<lp::LltcSnrTag>(snrTag);

  this->receiveNack(nack, endpointId, snr);
}
