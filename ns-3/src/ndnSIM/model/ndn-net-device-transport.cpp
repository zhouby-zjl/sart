...
#include "../ndn-cxx/ndn-cxx/lp/packet.hpp"
...
//  lp::Packet pkt(header.getBlock());
//  pkt.set<lp::LltcSnrField>((double) snr);
//  const Block& block = std::move(pkt.wireEncode());
//  this->receive(block);

  this->receive(std::move(header.getBlock()), 0, snr, rssi);
}
